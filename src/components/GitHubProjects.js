...
export default function GitHubProjects() {
    ...
    if (error) {
        return (
            <a href="https://github.com/jyoung9154" target="_blank" rel="noopener noreferrer"
               className="block p-8 rounded-2xl bg-white/5 border border-white/5 hover:border-[#0DA6F2]/30 transition-all text-center no-underline">
                <p className="text-slate-300 font-semibold mb-2">GitHub에서 전체 프로젝트 보기</p>
                <p className="text-slate-500 text-sm mb-0">일시적으로 저장소 목록을 불러올 수 없습니다 — github.com/jyoung9154</p>
            </a>
        );
    }
    if (!repos.length) {
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-4 text-xs text-slate-500 flex-wrap">
                                {repo.language && <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[#0DA6F2]" />{repo.language}</span>}
